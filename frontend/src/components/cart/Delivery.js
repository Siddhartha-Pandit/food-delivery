...
const Delivery = () => {
  ...
  // Set default values for state properties if deliveryInfo is undefined
  ...
  // useEffect to update state when deliveryInfo changes
  useEffect(() => {
    if (deliveryInfo) {
      setAddress(deliveryInfo.address);
      setCity(deliveryInfo.city);
      setPostalCode(deliveryInfo.postalCode);
